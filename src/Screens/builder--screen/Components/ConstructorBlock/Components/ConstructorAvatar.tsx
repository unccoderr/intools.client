...
export const ConstructorAvatar = () => {
	const { avatarURL, setAvatarURL, storiesCircle } = useContext(ProfileBuilderContext)

	const onChange = async (e: ChangeEvent<HTMLInputElement>) => {
		if (setAvatarURL) await updateSrc(e, setAvatarURL)
	}

    return <div className={`constructorBlock--statsAvatar${!avatarURL ? ' constructorBlock--statsAvatar-empty' : ''}${storiesCircle ? ' constructorBlock--statsAvatar-circle' : ''}`}>
        <label>
            <input type="file" onChange={onChange} accept=".jpg, .jpeg, .png"/>
            { !avatarURL && <svg id={'no-export'} width="24" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
