...
export const ConstructorStoryItem = ({ story, profileID, id }: ConstructorStoryItemProps) => {
	const { updateStory } = useBuilderProfilesData
    const [src, setSrc] = useState(story.src.toString())
	const [name, setName] = useState(story.name.toString())

	const onChangeSrc = (e: ChangeEvent<HTMLInputElement>) => {
		if (!e.target.files || e.target.files.length === 0) return
		getBase64Src(e.target.files[0])
			.then(base64URL => {
				setSrc(base64URL)
				updateStory(profileID, id, base64URL, 'src')
			})
	}
	const onChangeName = (e: ChangeEvent<HTMLInputElement>) => {
		setName(e.target.value)
		updateStory(profileID, id, e.target.value, 'name')
	}

    return <div className={'constructorBlock--storiesItem'}>
        <label>
