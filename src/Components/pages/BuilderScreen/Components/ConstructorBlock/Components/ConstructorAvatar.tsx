...
export const ConstructorAvatar = () => {
    ...
    return <div className={`constructorBlock--statsAvatar${!avatarURL ? ' constructorBlock--statsAvatar-empty' : ''}${!storiesCircle ? ' constructorBlock--statsAvatar-nocircle' : ''}`}>
        <label>
            <input type="file" onChange={onChange} accept=".jpg, .jpeg, .png"/>
            { !avatarURL && <div>
				<svg id={'no-export'} width="24" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
